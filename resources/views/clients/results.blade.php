									...
									<div class="table-responsive">
                                        ...
                                        <table id="datatable-2" class="table table-striped table-bordered w-100 text-nowrap display ">
                                                ...
                                                @foreach($clients as $key=>$client)
                                                    <input type="hidden" value="{{json_encode($client,true)}}" name="items[]"  >
                                                    
                                                    <tr>
                                                    <?php $designation = $client->ville.' '.$client->adresse; ?>
                                                    <?php $index=$key+1;// = $client->ville.' '.$client->adresse; ?>

                                                        <td> {{$index}}</td>
                                                        <td>
                                                        <input type="hidden" id="row-{{$key}}-designation" name="dynamic_form[dynamic_form][{{$key}}][designation]" value="{{$client->ville.' '.$client->adresse}}" />
                                                        {{ str_limit($client->ville.' '.$client->adresse, $limit = 80, $end = '...') ?? $client->designation }}
                                                        </td>
                                                        <td>
                                                            {{ $client->type_emballage ?? '' }}
                                                        </td>

                                                        <td>
                                                        <input type="hidden" name="dynamic_form[dynamic_form][{{$key}}][quantite]" value="{{$client->qte_facturee ?? '' }}" />
                                                        {{$client->qte_facturee ?? '' }}
                                                         </td>
                                                        <td>
                                                        ...
                                                        <input type="number" class="form-control prix_unitaire" 
                                                            ...
                                                            value="{{$client->prix ?? ''}}"
                                                            data-qte="{{$client->qte_facturee}}" data-key="{{$key}}" id="row-{{$key}}-age" name="dynamic_form[dynamic_form][{{$key}}][prix]"  ></td>
                                                        <td>
                                                        <input 
                                                        value="{{$client->total ?? ''}}"
