...
@section('content')
                    ...
                    <div class="page-header">
						<h4 class="page-title">Table des immobilisations  : </h4>
						<ol class="breadcrumb">
						...
						<div class="col-md-12 col-lg-12">
							<div class="card">
								<div class="card-header">
                                        <div class="col-md-12 col-lg-12">
                                            <form class="row" method="post" action="{{route('immobilisation.filter')}}" id="formfilter">
                                                @csrf
                                                @if($filter == 0)
                                                <div class="col-md-1">
                                            ...
                                            @if($filter==1)
                                            <form class="row" method="post" action="{{route('immobilisation.generer')}}" id="formgenerate">
                                                @csrf
                                                    ...
                                                    <input class="form-control" value="{{$type ?? ''}}" name="type" type="hidden">
									...
									<div class="table-responsive">
										<table id="datatable-5" class="table table-bordered key-buttons text-nowrap" >
                                            ...
                                            @foreach($immobilisations as $key=>$immobilisation)
                                                ...
                                                <td>{{$immobilisation->camion ?? '' }}</td>
                                                
                                                <td>{{$immobilisation->date_facture ?? '' }}</td>
                                                    <td>{{$immobilisation->mission ?? '' }}</td>
													<td>{{$immobilisation->ville ?? '' }}</td>
													<td>{{$immobilisation->wilaya ?? '' }}</td>
													<td>{{$immobilisation->somme_de_qte_facturee ?? '' }}</td>
												</tr>
                                            @endforeach
											</tbody>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
		<form action="{{route('immobilisation.create')}}" method="post" enctype="multipart/form-data">
			@csrf
